...
# Define the chatbot-based Streamlit app content again after reset
chatbot_app_code = """
...
st.set_page_config(page_title="Remnant Migration Chat", layout="centered")
st.title("Remnant Migration Chatbot")

# Initialize session state for chat history and user context fields
...
def ask_question(prompt_key, question_text):
    st.chat_message("assistant").markdown(question_text)
    user_input = st.chat_input("")
    if user_input:
        st.chat_message("user").markdown(user_input)
        st.session_state.context_data[prompt_key] = user_input
        return True
    return False
...
# Ask each question if not already answered
if not st.session_state.inputs_collected:
    for key, text in questions:
        if key not in st.session_state.context_data:
            if not ask_question(key, text):
                st.stop()
...
if st.session_state.inputs_collected:
    ...
    st.chat_message("assistant").markdown(f"Thanks {data['name']}, here's your migration advice:")
    st.chat_message("assistant").markdown(advice)
"""

# Save the chatbot app in the streamlit_app directory
streamlit_dir = Path("streamlit_app")
streamlit_dir.mkdir(exist_ok=True)
chatbot_file = streamlit_dir / "chat_migration.py"
chatbot_file.write_text(chatbot_app_code)

str(chatbot_file)  # Show path to chatbot app file
